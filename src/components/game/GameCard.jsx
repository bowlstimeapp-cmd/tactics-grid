...
export default function GameCard({ card, size = 'md', onClick, selected, showStats = true, effectiveStats, isFlipping, myPlayerNum = 1, faceDown = false, className = '' }) {
  ...
  const n = es ? es.north : card.north;
  const e = es ? es.east : card.east;
  const s = es ? es.south : card.south;
  const w = es ? es.west : card.west;

  const isModified = (dir) => es && es.mods && es.mods[dir] !== 0;
  const modColor = (dir) => {
    if (!es || !es.mods) return '';
    if (es.mods[dir] > 0) return 'text-emerald-400';
    if (es.mods[dir] < 0) return 'text-red-400';
    return '';
  };
            ...
            {showStats && (
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="relative" style={{ width: size === 'sm' ? 36 : size === 'md' ? 52 : 68, height: size === 'sm' ? 36 : size === 'md' ? 52 : 68 }}>
                  {/* North */}
                  <div className={`absolute top-0 left-1/2 -translate-x-1/2 ${statSizes[size]} flex items-center justify-center rounded-full bg-black/60 font-bold ${modColor('north')}`}>
                    {n}
                  </div>
                  {/* South */}
                  <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 ${statSizes[size]} flex items-center justify-center rounded-full bg-black/60 font-bold ${modColor('south')}`}>
                    {s}
                  </div>
                  {/* West */}
                  <div className={`absolute top-1/2 left-0 -translate-y-1/2 ${statSizes[size]} flex items-center justify-center rounded-full bg-black/60 font-bold ${modColor('west')}`}>
                    {w}
                  </div>
                  {/* East */}
                  <div className={`absolute top-1/2 right-0 -translate-y-1/2 ${statSizes[size]} flex items-center justify-center rounded-full bg-black/60 font-bold ${modColor('east')}`}>
                    {e}
                  </div>
                </div>
              </div>
