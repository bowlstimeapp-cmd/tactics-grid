...
export default function DeckBuilder() {
  ...
  const saveDeck = async () => {
    if (!deckName.trim() || deckCards.length !== 20) return;
    if (editingDeck) {
          ...
          {decks.map(deck => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-2">{cards.length}/20 cards</p>
              </motion.div>
            );
          })}
        </div>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-2xl bg-slate-900 border-amber-900/30 max-h-[90vh] overflow-y-auto">
          ...
          <p className="text-sm text-muted-foreground">
            Select 20 cards ({deckCards.length}/20)
